...
`session_change = recent_sessions / baseline_sessions - 1`

All observed session baselines are positive. A decline flag means `session_change <= -0.20`. This is an interpretable exploratory threshold, not an optimised or validated operating threshold.
...
Primary sources: `2026 UNSW DataSoc X Atlassian Datathon.pdf`, particularly pages 4-8; `README.md`; `customers.csv`; `customer_support_tickets.csv`; `product_usage.csv`, all supplied by the user. All empirical claims in this report are derived from those inputs. The solution design and scenario assumptions are original proposals, not additional facts supplied by the organisers.
